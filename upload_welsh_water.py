#!/usr/bin/env python3
"""Upload Dwr Cymru Welsh Water Drinking Water Quality CSV to Supabase.
X-rayed Determinand: Lead (10 - will apply 25.12.2013), Fluoride (Total), Nitrate (Total),
...
# Welsh Water CSV determinand names (x-rayed) -> (DB chemical name, unit)
TARGET_DETERMINANDS = {
...
CSV_PATH = "/Users/chrispennington/Downloads/Dwr_Cymru_Welsh_Water_Drinking_Water_Quality_2024.csv"
...
print("\nFetching existing Welsh Water zones...")
existing_raw = supabase_get("water_zones?supplier=eq.Welsh%20Water&select=zone_id&limit=10000")
existing_ids = {r["zone_id"] for r in existing_raw}
print(f"  {len(existing_ids)} Welsh Water zones already in DB")

new_zones = [
    {"zone_id": lsoa_cd, "zone_name": f"Welsh Water {lsoa_cd}", "supplier": "Welsh Water"}
