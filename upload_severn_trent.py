...
CSV_PATH = "/Users/chrispennington/Downloads/Severn_Trent_Water_Domestic_Water_Quality.csv"
...
print("\nFetching existing Severn Trent zones...")
existing_raw = supabase_get("water_zones?supplier=eq.Severn%20Trent%20Water&select=zone_id&limit=10000")
existing_ids = {r["zone_id"] for r in existing_raw}
print(f"  {len(existing_ids)} Severn Trent zones already in DB")

# ── Step 3: Insert missing water_zones ────────────────────────────────────────
new_zones = [
    {
        "zone_id": lsoa_cd,
        "zone_name": f"Severn Trent {lsoa_cd}",
