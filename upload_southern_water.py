#!/usr/bin/env python3
"""
Upload Southern Water data to Supabase.
CSV: Southern_Water_Domestic_Drinking_Water_Quality_2024.csv
Columns: Sample_ID, Sample_Date, Determinand, DWI_Code, Units, Operator, Result, LSOA, ObjectId
Uses LSOA as zone_id (LSOA codes, same format as postcodes.io).
"""
import csv
import json
...
# Southern Water uses: LEAD (UNFLUSHED), CHLORINE (FREE), CHLORINE (TOTAL), NITRATE, FLUORIDE
TARGET_DETERMINANDS = {
...
# Fallback if file is in water-data-audit
import os
...
print("\nFetching existing Southern Water zones...")
existing_raw = supabase_get("water_zones?supplier=eq.Southern%20Water&select=zone_id&limit=10000")
existing_ids = {r["zone_id"] for r in existing_raw}
print(f"  {len(existing_ids)} Southern Water zones already in DB")
