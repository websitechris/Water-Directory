...
# Map CSV determinand names -> DB chemical names (scorecard mapping)
TARGET_DETERMINANDS = {
...
print("Reading CSV...")
lsoa_names = {}       # lsoa_cd -> lsoa_nm
lsoa_vals = defaultdict(lambda: defaultdict(list))  # lsoa_cd -> chemical -> [values]

with open("/Users/chrispennington/Downloads/Anglian_Water_Domestic_Water_Quality_2024.csv", encoding="utf-8-sig") as f:
    reader = csv.DictReader(f)
    for row in reader:
        det = row["Determinand"].strip()
        if det not in TARGET_DETERMINANDS:
            continue
        lsoa_cd = row["LSOA21CD"].strip()
        lsoa_nm = row["LSOA21NM"].strip()
        lsoa_names[lsoa_cd] = lsoa_nm
        try:
...
print("\nFetching existing Anglian zones...")
existing_raw = supabase_get("water_zones?supplier=eq.Anglian%20Water&select=zone_id&limit=10000")
existing_ids = {r["zone_id"] for r in existing_raw}
print(f"  {len(existing_ids)} Anglian zones already in DB")

# ── Step 3: Insert missing water_zones (zone_id = LSOA code) ─────────────────
new_zones = [
    {
        "zone_id": lsoa_cd,
        "zone_name": lsoa_names[lsoa_cd],
...
for lsoa_cd, chemicals in lsoa_vals.items():
    for chemical_name, values in chemicals.items():
        ...
        # Find the unit for this chemical
        unit = next(
            unit for det, (chem, unit) in TARGET_DETERMINANDS.items()
