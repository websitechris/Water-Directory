#!/usr/bin/env python3
"""Upload Northumbrian Water Domestic Water Quality CSV to Supabase.
X-rayed Determinand: Lead, Fluoride, Nitrate, Free Chlorine, Total Chlorine.
"""
import csv
import json
...
# Northumbrian CSV determinand names (x-rayed) -> (DB chemical name, unit)
TARGET_DETERMINANDS = {
    "Lead":           ("Lead", "μg/l"),
...
CSV_PATH = "/Users/chrispennington/Downloads/Northumbrian_Water_Domestic_Water_Quality_2022-2024.csv"
...
print("\nFetching existing Northumbrian Water zones...")
existing_raw = supabase_get("water_zones?supplier=eq.Northumbrian%20Water&select=zone_id&limit=10000")
existing_ids = {r["zone_id"] for r in existing_raw}
print(f"  {len(existing_ids)} Northumbrian Water zones already in DB")

new_zones = [
    {"zone_id": lsoa_cd, "zone_name": f"Northumbrian Water {lsoa_cd}", "supplier": "Northumbrian Water"}
