#!/usr/bin/env python3
"""Upload United Utilities Domestic Drinking Water Quality CSV to Supabase.
X-rayed DETERMINAND: Lead, Fluoride, Nitrate, Residual chlorine - Free/Total.
"""
import csv
import json
...
# United Utilities CSV determinand names (x-rayed) -> (DB chemical name, unit)
TARGET_DETERMINANDS = {
    "Lead":                       ("Lead", "μg/l"),
    "Fluoride":                   ("Fluoride", "mg/l"),
    "Nitrate":                    ("Nitrate", "mg/l"),
    "Residual chlorine - Free":   ("Chlorine (Residual)", "mg/l"),
    "Residual chlorine - Total":  ("Chlorine (Total)", "mg/l"),
}
...
CSV_PATH = "/Users/chrispennington/Downloads/United_Utilities_Domestic_Drinking_Water_Quality_2023-2024.csv"
...
print("\nFetching existing United Utilities zones...")
existing_raw = supabase_get("water_zones?supplier=eq.United%20Utilities&select=zone_id&limit=10000")
existing_ids = {r["zone_id"] for r in existing_raw}
print(f"  {len(existing_ids)} United Utilities zones already in DB")

new_zones = [
    {"zone_id": lsoa_cd, "zone_name": f"United Utilities {lsoa_cd}", "supplier": "United Utilities"}
