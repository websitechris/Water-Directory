#!/usr/bin/env python3
"""Upload Wessex Water Domestic Water Quality CSV to Supabase.
X-rayed Determinand: Lead (10), Fluoride (total), Nitrate (total), Residual disinfectant - free/total.
"""
import csv
import json
...
print("\nFetching existing Wessex Water zones...")
existing_raw = supabase_get("water_zones?supplier=eq.Wessex%20Water&select=zone_id&limit=10000")
existing_ids = {r["zone_id"] for r in existing_raw}
print(f"  {len(existing_ids)} Wessex Water zones already in DB")

new_zones = [
    {"zone_id": lsoa_cd, "zone_name": f"Wessex Water {lsoa_cd}", "supplier": "Wessex Water"}
